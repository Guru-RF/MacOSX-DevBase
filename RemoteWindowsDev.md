...
> noted. Reconstructed from the `WINSUCKS` setup history.
...
```powershell
Rename-Computer -NewName "winsucks" -Restart
```
...
git clone git@github.com:Guru-RF/SVXConnect-Windows.git
git clone git@github.com:Guru-RF/SVXConnect-OSX.git

code .\SVXConnect-Windows\
```

### Run / package

```powershell
cd ~\Git\Guru-RF\SVXConnect-Windows

# Run the app
dotnet run --project src\SVXConnect.App
...
Get-AppxPackage *RFGuru.SVXConnect* | Remove-AppxPackage
```
...
```sshconfig
Host winsucks
    HostName winsucks.local          # or the host's IP / WireGuard address
    User ure
```

Then in VS Code on the Mac:

1. **Cmd-Shift-P → Remote-SSH: Connect to Host… → `winsucks`**
2. Open `~/Git/Guru-RF/SVXConnect-Windows` on the remote.
...
> `C:\Users\ure\.ssh\authorized_keys` (and, for admin users,
